...
import kotlinx.android.synthetic.main.layout_btn_favorite.*
...
class DetailActivity : AppCompatActivity() {
    ...
    private fun setupTabLayout(data: UserDetail) {
        ...
        actionAdapter = ViewPagerAdapter(this, data)
        binding?.apply {
            viewpager.adapter = actionAdapter
            TabLayoutMediator(tab, viewpager) { tab, pos ->
                tab.text =
                    StringBuilder("${arrayCount[pos]?.let { converterNumber(it) }}\n${tabTitle[pos]}")
            }.attach()
        }
    }
