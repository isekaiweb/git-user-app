    ...
    private suspend fun safeGetDetailUser(username: String) {
        ...
        try {
            if (hasInternetConnection()) {
                ...
                detailUser.postValue(Resource.Error("No internet connection"))
            }

        } catch (e: Throwable) {
            when (e) {
                is IOException -> detailUser.postValue(Resource.Error("Network Failure"))
                else -> detailUser.postValue(Resource.Error("Conversion Error"))
            }
        }
    }
    ...
    private suspend fun safeGetDetailTypeUser(username: String, action: String) {
        ...
        try {
            if (hasInternetConnection()) {
                ...
                detailTypeUser.postValue(Resource.Error("No internet connection"))
            }

        } catch (e: Throwable) {
            when (e) {
                is IOException -> detailTypeUser.postValue(Resource.Error("Network Failure"))
                else -> detailTypeUser.postValue(Resource.Error("Conversion Error"))
            }
        }
    }
